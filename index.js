...
// Si existe la variable DB_PATH (ej. en el disco de Render), la usa. Si no, usa local.
...
// 🔥 CONFIGURACIÓN DE LAS CUENTAS DE GMAIL (Mapa O(1) ultra rápido)
...
<style>
    ...
    body::before {
        ...
        background: radial-gradient(circle at 50% 50%, rgba(255,23,68,0.06), rgba(0,230,118,0.06), rgba(17,60,207,0.06), rgba(138,43,226,0.06), rgba(244,117,33,0.06), transparent 60%);
        animation: rotateLED 20s linear infinite; z-index: -1; pointer-events: none;
    }
    ...
    .sidebar { 
        width: 280px; background: var(--bg-sidebar); backdrop-filter: blur(25px); 
    ...
    /* Scrollbar estético para sidebar */
    .sidebar::-webkit-scrollbar { width: 5px; }
    .sidebar::-webkit-scrollbar-thumb { background: #333; border-radius: 10px; }
    
    .sidebar-title { color: #666; font-size: 10px; text-transform: uppercase; letter-spacing: 2px; margin: 25px 0 10px 15px; font-weight: 800; }
    ...
    .tab-btn:hover { background: rgba(255, 255, 255, 0.03); color: var(--text-primary); transform: translateX(4px); }
    .tab-btn.active { 
    ...
    button.action-btn, .action-btn-link { 
        ...
        text-decoration: none; border: 1px solid rgba(255,255,255,0.1); position: relative; overflow: hidden;
    }
    button.action-btn::after { content: ''; position: absolute; top: 0; left: -100%; width: 50%; height: 100%; background: linear-gradient(90deg, transparent, rgba(255,255,255,0.2), transparent); transition: 0.5s; }
    button.action-btn:hover::after { left: 100%; }
...
app.get('/', (req, res) => {
    ...
    <style>
        ...
        body::before {
            content: ""; position: absolute; width: 300px; height: 300px; background: var(--mx-green); 
            filter: blur(150px); opacity: 0.15; top: 10%; left: 10%; animation: float 10s infinite alternate;
        }
        body::after {
            content: ""; position: absolute; width: 300px; height: 300px; background: var(--mx-red); 
            filter: blur(150px); opacity: 0.15; bottom: 10%; right: 10%; animation: float 10s infinite alternate-reverse; z-index: -1;
        }
        @keyframes float { from { transform: translate(0, 0); } to { transform: translate(50px, 50px); } }
        
        .login-panel { position: relative; background: var(--bg-panel); backdrop-filter: blur(40px); padding: 70px 50px; border-radius: 24px; border: 1px solid rgba(255, 255, 255, 0.05); max-width: 440px; width: 90%; text-align: center; box-shadow: 0 30px 80px rgba(0,0,0,0.9), inset 0 1px 0 rgba(255,255,255,0.1); box-sizing: border-box; z-index: 10; }
        ...
        input { width: 100%; padding: 20px; margin-bottom: 20px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.1); background: rgba(0,0,0,0.6); color: white; box-sizing: border-box; font-size: 15px; font-weight: 500; transition: 0.3s; box-shadow: inset 0 2px 10px rgba(0,0,0,0.5); }
        input:focus { border-color: var(--mx-green); outline: none; background: rgba(0,0,0,0.9); box-shadow: 0 0 20px rgba(0,230,118,0.2), inset 0 2px 10px rgba(0,0,0,0.5); }
        ...
        .lang-selector-login select { background: rgba(18,18,18,0.9); color: white; border: 1px solid rgba(255,255,255,0.1); padding: 10px 15px; border-radius: 10px; font-family: 'Inter', sans-serif; font-weight: 600; font-size: 13px; cursor: pointer; outline: none; transition: 0.3s; }
