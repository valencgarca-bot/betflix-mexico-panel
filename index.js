...
// 🔥 ESTILOS UI (Ocultos para ahorrar espacio en la vista, se mantienen igual) 🔥
const CSS_MODERNO = `
<style>
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap');
    :root { --bg-deep: #050505; --bg-panel: rgba(18, 18, 18, 0.85); --bg-sidebar: rgba(10, 10, 10, 0.9); --text-primary: #ffffff; --text-secondary: #b0b0b0; --mx-green: #00e676; --mx-red: #ff1744; --mx-white: #f5f5f5; --border-color: rgba(60, 60, 60, 0.3); }
    .goog-te-banner-frame.skiptranslate { display: none !important; } body { top: 0px !important; } #goog-gt-tt { display: none !important; } .goog-te-gadget-tooltip { display: none !important; }
    .custom-lang-select { background: rgba(0,0,0,0.8); color: white; border: 1px solid #333; padding: 6px 12px; border-radius: 8px; font-family: 'Inter', sans-serif; font-size: 13px; font-weight: 600; cursor: pointer; outline: none; transition: 0.2s; }
...
app.get('/', (req, res) => {
    ...
    <style>
        @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap');
        :root { --bg-deep: #0a0a0a; --bg-panel: rgba(18, 18, 18, 0.95); --mx-green: #00e676; --mx-red: #ff1744; --mx-white: #f5f5f5; }
        ...
        .goog-te-banner-frame.skiptranslate { display: none !important; } body { top: 0px !important; } #goog-gt-tt { display: none !important; }
...
app.get('/admin/logout-todos', (req, res) => {
    ...
    if (esAdminPrincipal) { req.sessionStore.clear((err) => { res.send("<script>alert('✅ Se ha cerrado la sesión de TODOS los usuarios conectados.'); window.location='/';</script>"); }); } 
    ...
    if (esAdminPrincipal || esSubAdmin) {
        ...
        try {
            ...
            subadmins.forEach(sub => {
                ...
                <details class="folder item-folder">
                    <summary>
                        ...
                        <div style="display:flex; align-items:center; gap:12px;"><span class="user-count">${clientesDelSub.length} usuarios</span></div>
                    </summary>
            ...
            let registrosHtml = `<div style="overflow-x:auto;"><table style="width:100%; border-collapse: collapse; font-size: 14px; margin-top:20px;">
                <tr style="border-bottom: 1px solid var(--border-color); color:var(--text-secondary); text-align:left;">
                    <th style="padding:12px;">Usuario</th><th style="padding:12px;">Correo Buscado</th><th style="padding:12px;">Fecha y Hora</th>
                </tr>`;
            ...
            } else { registrosHtml += `<tr><td colspan="3" style="padding:15px; text-align:center; color:#555;">No hay registros.</td></tr>`; }
            ...
            <div class="dashboard-layout">
                ...
                <div class="main-content">
                    ...
                    <div id="panel-usuarios" class="tab-panel">
                        ...
                        ${(!subadminsHtml && !clientesDirectosHtml) ? '<p style="color:#555; text-align:center;">No hay usuarios registrados aún.</p>' : ''}
                    </div>
                    <div id="panel-auditoria" class="tab-panel">
                        <div class="panel-header"><h3>🕵️ Auditoría de Códigos</h3></div>
                        ${registrosHtml}
                    </div>
                </div>
            </div></body>`);
        ...
        <div style="padding: 60px 20px; display:flex; justify-content:center;">
            <div class="tab-panel active" style="margin: 0; width:100%; max-width:600px; display: block; padding: 40px;">
                <div class="panel-header" style="text-align:center;"><h3>📨 Leer Correos</h3></div>
                <form action="/buscar" method="POST">
...
app.get('/admin/del-user/:id', async (req, res) => {
    try { await dbRun("DELETE FROM usuarios WHERE id = ?", [req.params.id]); await dbRun("DELETE FROM correos WHERE user_id = ?", [req.params.id]); res.redirect('/dash'); } catch(err) { res.redirect('/dash'); }
});

app.get('/admin/del-mail/:id', async (req, res) => {
    try { await dbRun("DELETE FROM correos WHERE id = ?", [req.params.id]); res.redirect('/dash'); } catch(err) { res.redirect('/dash'); }
});

// 🔥 SISTEMA DE BÚSQUEDA ULTRA RÁPIDO (X-GM-RAW + DESCARGA ÚNICA) 🔥
app.post('/buscar', async (req, res) => {
    ...
    try {
        ...
        // 1. TU LÓGICA DE ENRUTAMIENTO ESTRICTO
        // Por defecto, si no cumple nada o es un dominio raro, se va a esta bandeja:
        ...
        if (correoIngresado.includes('+')) {
            ...
            if (partes.length === 2) {
                ...
                const correoBase = `${usuarioPuro}@${partes[1]}`; // Ej: clubecampestrejp@gmail.com

                // Si este correo base existe en tu mapa, lo seleccionamos, ignorando a Darci.
                if (CUENTAS_GMAIL_MAP[correoBase]) {
                    correoSeleccionado = correoBase;
                }
            }
        } else {
            // Por si acaso el cliente busca el correo base tal cual (sin el +)
            if (CUENTAS_GMAIL_MAP[correoIngresado]) {
        ...
        try {
            ...
            // 🔥 EL SECRETO DE LA VELOCIDAD: BÚSQUEDA NATIVA Y CERO DESCARGAS 🔥
            // Paso A: Pedimos A GMAIL buscar (X-GM-RAW es instantáneo) y solo le pedimos que nos devuelva los IDs, NO el contenido.
            const resultadosBusqueda = await connection.search([['X-GM-RAW', correoIngresado]], { bodies: ['HEADER.FIELDS (DATE)'], struct: false });

            if (resultadosBusqueda.length > 0) {
                cuentaExitosa = correoSeleccionado;
                
                // Paso B: Ordenamos para saber cuál es el mensaje MÁS RECIENTE
                resultadosBusqueda.sort((a, b) => b.attributes.uid - a.attributes.uid);
                const uidMasReciente = resultadosBusqueda[0].attributes.uid;

                // Paso C: AHORA SÍ, descargamos EL TEXTO PERO SOLAMENTE DE ESE ÚNICO CORREO (1 solo de 50)
                messages = await connection.search([['UID', uidMasReciente]], { bodies: [''], struct: true });
            }
        } catch (err) {
            console.log(`⚠️ Error al conectar con ${correoSeleccionado}:`, err.message);
        }

        // Si no encontró resultados en la bandeja asignada
        ...
        // Parseamos el único correo que descargamos
        ...
        if (accion === 'pais') {
            ...
            const reglasPais = [
                ...
                { id: "🇳🇱 Sede Europea", keys: ['netherlands', 'holanda', 'países bajos', 'amsterdam', 'international b.v.', 'b.v.'] }
            ];
        ...
        if (accion === 'ip') {
            ...
            return res.send(`<div style="background:#000; text-align:center; padding:15px;"><a href="/dash" style="color:#fff; text-decoration:none; border: 1px solid #fff; padding: 8px 15px; border-radius: 5px; font-family:'Inter', sans-serif;">⬅ VOLVER AL PANEL</a></div><div style="background:#111; color:white; padding: 40px; text-align:center; font-family:'Inter', sans-serif; min-height:100vh;"><h2>📡 Registro de IP</h2><p>Correo: <strong style="color:var(--mx-green);">${email_search}</strong> (Bandeja: ${cuentaExitosa})</p><div style="margin: 40px auto; padding: 30px; background:#222; border-radius:15px; display:inline-block; border: 1px solid #f00;">${ipContenido}</div></div>`);
        }

        // 🔥 REGISTRAR CÓDIGOS SOLICITADOS
        ...
        res.send(`<div style="background:#000; text-align:center; padding:15px;"><a href="/dash" style="color:#fff; text-decoration:none; border: 1px solid #fff; padding: 8px 15px; border-radius: 5px; font-family:'Inter', sans-serif;">⬅ VOLVER AL PANEL</a></div><div style="background:white; color:black; padding: 20px; margin: 0 auto; max-width: 800px; font-family:'Inter', sans-serif;"><p style="text-align:center; font-weight:bold; color:red;">Encontrado en bandeja: ${cuentaExitosa}</p>${contenidoFinal}</div>`);
    } catch (e) { 
