...
app.post('/buscar', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            // 🔥 OPTIMIZACIÓN DE VELOCIDAD 1: Búsqueda indexada por 'TO' (Destinatario) en lugar de 'TEXT'
            // 🔥 OPTIMIZACIÓN DE VELOCIDAD 2: Solo pedimos los encabezados primero para no saturar la memoria
            let searchResults = await connection.search([['TO', correoIngresado]], { bodies: ['HEADER'] });

            if (searchResults.length > 0) {
                cuentaExitosa = correoSeleccionado;
                
                // Ordenar de inmediato para obtener el más reciente (mayor UID)
                searchResults.sort((a, b) => b.attributes.uid - a.attributes.uid);
                let latestUid = searchResults[0].attributes.uid;
                
                // 🔥 OPTIMIZACIÓN DE VELOCIDAD 3: Descargamos EL CUERPO COMPLETO únicamente del correo más reciente
